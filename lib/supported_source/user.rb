...
module SupportedSource
  class User
    ...
    def self.readme_filename
      "#{ SupportedSource.user_supso_config_root }/README.txt"
    end
    ...
    def self.attach_to_email!(email)
      ...
      response = Util.http_post("#{ SupportedSource.supso_api_root }users/attach", data)
    ...
    def self.log_in_with_password!(email, password)
      ...
      response = Util.http_post("#{ SupportedSource.supso_api_root }sign_in", data)
    ...
    def self.log_in_with_confirmation_token!(email, confirmation_token)
      ...
      response = Util.http_post("#{ SupportedSource.supso_api_root }users/confirm", data)
    ...
    def self.log_out!
      if File.exists?(User.current_user_filename)
        ...
        if user && user.auth_token
          data = {
              version: SupportedSource::VERSION,
              auth_token: user.auth_token,
              user_id: user.id,
          }

          response = Util.http_post("#{ SupportedSource.supso_api_root }sign_out", data)
    ...
    def self.save_user_supso_readme!
      if !File.exists?(User.readme_filename)
        readme_contents = File.open("#{ SupportedSource.gem_root }/lib/templates/user_dir_readme.txt", 'r').read
